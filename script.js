...
function getProfile(data) {
  ...
  profileInfo.innerHTML = `<div class="card">
   ...
   <div class="card-text">
   <p>${data.bio}</p>
   <p><i class="fa-solid fa-user-group" style="color: #ff0000;"></i>${data.followers} Followers ${data.following} Following</p>
   <p><i class="fa-solid fa-location-dot" style="color: #ff0000;"></i>${data.location}</p>
   <button>
   <a href=${data.html_url} target="_blank">Vist Profile</a>
   </button>
   </div>
   </div>
   </div>`
